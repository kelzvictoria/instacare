...
class Details extends Component<DetailsProps>{
        ...
        filterDropdown: (data: FilterProps) => (
            <div style={{ padding: 8 }}>
                <Input
                    ...
                    onPressEnter={() => this.handleSearch(data.selectedKeys, data.confirm)}
              ...
              Search
                ...
                <Button onClick={() => this.handleReset(data.clearFilters)} size="small" style={{ width: 90 }}>
              Reset
                </Button>
            </div>
        ),
    ...
    UNSAFE_componentWillMount() {
        ...
        this.props.dispatch({ type: UPDATE_PRICE, price:JSON.parse(predetails).price * 100});
    }
callback=()=>{
...
notGettingProvidersView = () => {
    ...
    this.props.dispatch({ type: UPDATE_NOTGETTINGPROVIDERS, status: notGettingProviders});
...
callb = (key)=> {
  ...
  handleInput = (e: any) => {
    this.props.dispatch({type: actions.UPDATE_TEXT_RESPONSE, data: {key: e.target.name, value: e.target.value}});
}
            ...
            <Row gutter={8}>
                ...
                <div style={{marginBottom:"2.5rem"}}>
                        <Link to={{
                            ...
                            <Button size="small"onClick={this.notGettingProvidersView} type="default" className={styles.toLeft}><span style={{float:"left"}}><Icon type="left" /> <Divider type="vertical"></Divider>Back</span></Button>
                        </Link>
                ...
                <Modal open={open} onClose={this.onCloseModal} center>
                  <Col xs={24}>
                      <h2 style={{textAlign:"center"}}>To buy {this.details.name.toLowerCase()} , please fill the form below</h2>
                     </Col>
                     <Col xs={12} style={{ marginBottom: "1rem" }}>
                          <Form.Item colon={false} label="Enter first name" style={{textAlign: "left"}}>
                              <Input size="large" onChange={this.handleInput} type="text" name="firstName" placeholder="First Name" defaultValue={this.props.quiz.responses.firstName} />
                          </Form.Item>
                      </Col>
                      <Col xs={12} style={{ marginBottom: "1rem" }}>
                          <Form.Item colon={false} label="Enter last name" style={{textAlign: "left"}}>
                              <Input size="large" onChange={this.handleInput} type="text" name="lastName" placeholder="Last Name" defaultValue={this.props.quiz.responses.lastName} />
                          </Form.Item>
                      </Col>
                      <Col xs={24} style={{ marginBottom: "1rem" }}>
                          <Form.Item required={true} colon={false} label="Email address" style={{ textAlign: "left" }}>
                              <Input size="large" onChange={this.handleInput} type="email" name="email" placeholder="someone@example.com" defaultValue={this.props.quiz.responses.email} />
                          </Form.Item>
                      </Col> 
                    ...
                    {notGettingProviders ?
                        <Row gutter={8}>
                         ...
                         <Card
                                ...
                                <img
                                    alt="example"
                                    src="../images/individual.svg" height="200px"
                                />
                                }
                                >
                                <Meta
                            ...
                            <Card
                                ...
                                >
                                <Meta
                                title="This Plan Covers Two People"
                        ...
                        <Collapse defaultActiveKey={['1']} onChange={this.callb}>
                            <Panel header="Benefits of this plan" key="1">
                                <h3>Features <span style={{float:"right"}}>Limits</span></h3>
                                <p>Malaria Treatment</p>
                                <p>Typhoid Treatment</p>
                                {this.AllServices[0]!=0?
                                <p><span style={{width:"80%"}}>Accidents and Emergencies</span> <span style={{float:"right"}}>{formatAsCurrency(this.AllServices[0])}</span></p>
                                : console.log("emergency zero")
                                }
                                {this.AllServices[1]?
                                this.AllServices[1].limit>0?<p><span style={{width:"80%"}}>Ambulance Services</span>  <span style={{float:"right"}}>{formatAsCurrency(this.AllServices[1].limit)}</span></p>:<p>Ambulance Services</p>
                                    : console.log("emergency zero")
                                }
                               {this.AllServices[2]?
                                <p  style={{width:"80%"}}>Cancer Care - {this.covers.cancerCare.offerings.join(" , ")}</p>
                                : console.log("no cancer care")
                               }
                               {this.AllServices[3] && this.AllServices[3] != null?
                                <p  style={{width:"80%"}}>Chronic Disease Medication</p>
                                : console.log("cancer care")
                               }
                               {this.AllServices[4]?
                                <p style={{width:"80%"}}>Consultations - {this.covers.consultations.join(" ,")}</p>
                               ...
                               {this.AllServices[6]?
                                <p style={{width:"80%"}} >Dental Options - {this.covers.dentalOptions.offerings.join(" , ")}</p>
                                : console.log("No dental")
                               }
                               {this.AllServices[7] && typeof(this.AllServices[7])!=null?
                               this.covers.diagnostics.offerings[1] && this.covers.diagnostics.limit ?
                                <p  style={{width:"80%"}}><span>Diagnostics</span> - {this.covers.diagnostics.offerings[0]},  
                                {this.covers.diagnostics.offerings[1]}, <span style={{float:"right"}}> {formatAsCurrency(this.covers.diagnostics.limit)}</span></p> :
                                <p  style={{width:"80%"}}><span>Diagnostics</span> - {this.covers.diagnostics.offerings[0]}</p>
                                : console.log("dental")
                               }
                               {this.AllServices[8]?
                                <p>Dialysis</p>
                               ...
                               this.AllServices[9].length>0?
                                <p style={{width:"80%"}}>Family Planning Services - {this.covers.familyPlanningServices.join(" , ")}</p>
                               ...
                               {this.AllServices[11]?
                                <p style={{width:"80%"}}>Immunizations - {this.covers.immunizations.offerings.join(" , ")}</p>
                                : console.log("immunization")
                               }
                                {this.AllServices[12] && this.AllServices[12].length>0?
                                <p  style={{width:"80%"}}>Natal Care - {this.covers.natalCare.join(" , ")}</p>
                                : console.log("natal")
                               }
                               {this.AllServices[13] && this.AllServices[13].length>0?
                                <p  style={{width:"80%"}}>Optical Options - {this.covers.opticalOptions.offerings.join(" , ")}</p>
                                : console.log("immunization")
                               }
                               {
                                
                                this.AllServices[14]>0?
                                <p>Pharmacy  <span style={{float:"right"}}>{formatAsCurrency(this.covers.pharmacyCover)}</span></p>   
                                :console.log("pharm")
                               }
                            </Panel>
                    </Collapse>
                ...
                <Card
                        hoverable
                        style={{ width: "100%" , backgroundColor:"#EEEEEE", textAlign:"center"}}
                        cover={<img alt="example" style={{margin:"20px auto", maxWidth:"70%"}} src={this.details.imageUrl} />}
                    >
                        <Meta title={this.details.name} style={{textAlign:"center", marginBottom:"2%",color:"white"}} />

                         <p style={{fontSize:"3rem"}}>{formatAsCurrency(this.details.price)}</p>
                         <Button size="large" type="primary" onClick={this.onOpenModal}>Buy Plan</Button>
                </Card>
